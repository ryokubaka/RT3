    ...
    try:
        ...
        if conn:
            ...
            conn.search(
                ...
                attributes=list(authenticator.attributes.values())
            )
            
            if conn.entries:
                print(f"✅ Found user: {test_username}")
                print(f"  DN: {conn.entries[0].entry_dn}")
                for attr, value in conn.entries[0].entry_attributes.items():
                    if value:
                        print(f"  {attr}: {value[0] if isinstance(value, list) else value}")
            else:
